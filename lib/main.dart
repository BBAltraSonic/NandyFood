...
Future<void> main() async {
  ...
  AppLogger.info('Card payments can be added later via payment gateway integration');
  ...
  try {
    ...
    AppLogger.success('Database service initialized',
...
GoRouter createRouter() {
    ...
    final router = GoRouter(
      ...
      redirect: (BuildContext context, GoRouterState state) {
        ...
        AppLogger.navigation(location, isAuthenticated ? 'Authenticated' : 'Guest');
        ...
        final requiresAuth = protectedRoutes.any((route) => location.startsWith(route));

        if (requiresAuth && !isAuthenticated) {
          AppLogger.info('Protected route accessed - redirecting to login', data: {
            'requested': location,
          });
        ...
        if (isAuthenticated && location.startsWith('/auth')) {
          AppLogger.info('Authenticated user on auth screen - redirecting to home');
          return '/home';
        }
...
class _FoodDeliveryAppState extends ConsumerState<FoodDeliveryApp> {
  ...
  void initState() {
    super.initState();
    AppLogger.lifecycle('FoodDeliveryApp.initState', details: 'Creating router and initializing app state');
    _router = createRouter();
    AppLogger.success('Router created with ${_router.configuration.routes.length} routes');
  }
  ...
  Widget build(BuildContext context) {
    ...
    AppLogger.info('Building app with theme', data: {
  ...
  ThemeData _buildLightTheme() {
    ...
    return ThemeData(
      ...
      textTheme: const TextTheme(
        ...
        bodyLarge: TextStyle(fontSize: 16, fontWeight: FontWeight.normal, color: Color(0xFF4A4A4A)),
        bodyMedium: TextStyle(fontSize: 14, fontWeight: FontWeight.normal, color: Color(0xFF6A6A6A)),
        bodySmall: TextStyle(fontSize: 12, fontWeight: FontWeight.normal, color: Color(0xFF8A8A8A)),
      ),
  ...
  ThemeData _buildDarkTheme() {
    ...
    return ThemeData(
      ...
      textTheme: const TextTheme(
        ...
        bodySmall: TextStyle(fontSize: 12, fontWeight: FontWeight.normal, color: Color(0xFF909090)),
      ),
      appBarTheme: const AppBarTheme(
        centerTitle: true,
        elevation: 0,
