...
void main() {
  group('RestaurantProvider Tests', () {
    ...
    test('toggleDietaryRestriction should add restriction to selected list', () {
      // Initially no restrictions
      expect(container.read(restaurantProvider).selectedDietaryRestrictions, isEmpty);
      
      // Add vegetarian restriction
      notifier.toggleDietaryRestriction('vegetarian');
      
      // Verify vegetarian is added
      expect(container.read(restaurantProvider).selectedDietaryRestrictions, contains('vegetarian'));
    });
    
    test('toggleDietaryRestriction should remove restriction from selected list', () {
      // Add a restriction first
      notifier.toggleDietaryRestriction('vegan');
      expect(container.read(restaurantProvider).selectedDietaryRestrictions, contains('vegan'));
      
      // Remove the restriction
      notifier.toggleDietaryRestriction('vegan');
      
      // Verify it's removed
      expect(container.read(restaurantProvider).selectedDietaryRestrictions, isNot(contains('vegan')));
    });
    
    test('applyMenuItemFilters should filter menu items based on dietary restrictions', () {
      ...
      final itemsToFilter = notifier.state.menuItems.where(
        (item) => item.restaurantId == restaurant.id
      ).toList();
      
      final filtered = itemsToFilter.where((item) {
        return ['vegetarian'].every((restriction) =>
