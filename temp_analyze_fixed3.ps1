...
# Import common functions - adjust path based on script location
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$CommonScriptPath = Join-Path $ScriptDir '.specify/scripts/powershell/common.ps1'
if (-not (Test-Path $CommonScriptPath)) {
    # If script is run from a feature directory, common.ps1 would be in the project root
    $CommonScriptPath = Join-Path (Get-Location).Path '.specify/scripts/powershell/common.ps1'
    if (-not (Test-Path $CommonScriptPath)) {
        # Last attempt - look relative to current script location
        $CommonScriptPath = Join-Path $PSScriptRoot '.specify/scripts/powershell/common.ps1'
    }
}
...
try {
    $checkPrereqScript = Join-Path (Split-Path -Parent $CommonScriptPath) 'check-prerequisites.ps1'
...
$coverageHeader2 = "${pipe}${dash}${dash}${dash}${pipe}${dash}${dash}${dash}${pipe}${dash}${dash}${pipe}${dash}${dash}${dash}${dash}${pipe}"
...
Write-Output "* Coverage %: $(if($requirements.Count -gt 0) { [math]::Round((($requirements.Count - $reqCoverage | Where-Object {$_.HasTask -eq 'No'}).Count) / $requirements.Count * 10, 2) } else { 0 })%"
