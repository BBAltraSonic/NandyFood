...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'favourites') THEN
    RAISE NOTICE '✓ Table favourites created successfully';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'feedback') THEN
    RAISE NOTICE '✓ Table feedback created successfully';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE NOTICE '✓ Orders table updated with cancellation fields';
  END IF;
END $$;
...
DROP FUNCTION IF EXISTS get_user_active_devices;
DROP FUNCTION IF EXISTS get_user_favourite_restaurants;
